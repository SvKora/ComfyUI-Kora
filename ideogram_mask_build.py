"""Ideogram 4 prompt and mask builder.
"""
...
class Ideogram_mask_build_Kora(io.ComfyNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="Ideogram_mask_build_Kora",
            display_name="Ideogram Mask Build (Kora)",
            category="Kora/Ideogram",
            search_aliases=["ideogram", "mask" "caption", "bbox", "prompt builder", "kora", "inpaint"],
            is_experimental=True,
            description="""
Visual prompt builder + Mask Generator for Ideogram 4...
...
- brightness slider, token estimate, and Copy / Paste / Clear all""",
            inputs=[
                ...
                io.Int.Input("add_mask_to_background", default=0, min=0, max=100 ),
                ...
                io.Int.Input("bg_brightness", default=100, min=0, max=100, socketless=True, advanced=True,
                             tooltip="Background image brightness % (managed by the node UI slider)."),
                io.String.Input("sketch_data", default="[]", socketless=True, advanced=True),
    ...
    def execute(cls, width, height, background, style,
                ...
                bboxes=None, image=None, bg_brightness=100, add_mask_to_background=0, sketch_data="[]") -> io.NodeOutput:
        ...
        # 1. KORA
        bg_val = add_mask_to_background / 100.0
        mask_tensor = torch.full((1, height, width), bg_val, dtype=torch.float32)

        if boxes:
            for box in boxes:
                if not isinstance(box, dict) or box.get("nobbox"): continue
                
                x1 = max(0, min(width, int(box.get("x", 0) * width)))
                y1 = max(0, min(height, int(box.get("y", 0) * height)))
                x2 = max(0, min(width, int((box.get("x", 0) + box.get("w", 0)) * width)))
                y2 = max(0, min(height, int((box.get("y", 0) + box.get("h", 0)) * height)))
                
                #  Mask kora
                if x2 > x1 and y2 > y1:
                    mask_enabled = box.get("mask_enabled", False)
                    bbox_opacity = box.get("mask_opacity", 0)
                    bbox_blur = box.get("mask_blur", 0) # НОВЕ ПОЛЕ
                    
                    if mask_enabled and bbox_opacity > 0:
                        bbox_img = Image.new("L", (width, height), 0)
                        d = ImageDraw.Draw(bbox_img)
                        d.rectangle([x1, y1, x2, y2], fill=int(255 * (bbox_opacity / 100.0)))
                        
                        # Застосовуємо розмиття BBox
                        if bbox_blur > 0:
                            bbox_img = bbox_img.filter(ImageFilter.GaussianBlur(bbox_blur))
                            
                        bbox_tensor = torch.from_numpy(np.array(bbox_img, dtype=np.float32) / 255.0).unsqueeze(0)
                        mask_tensor = torch.maximum(mask_tensor, bbox_tensor)

                # Б) KORA
                brush_b64 = box.get("brush_base64", "")
                brush_op = box.get("brush_opacity", 100)
                
                if brush_b64 and brush_b64.startswith("data:image"):
                    try:
                        header, encoded = brush_b64.split(",", 1)
                        img_data = base64.b64decode(encoded)
                        drawn_img = Image.open(BytesIO(img_data)).convert("L")
                        if drawn_img.size != (width, height):
                            drawn_img = drawn_img.resize((width, height), Image.LANCZOS)
                        
                        drawn_np = np.array(drawn_img, dtype=np.float32) / 255.0
                        drawn_tensor = torch.from_numpy(drawn_np).unsqueeze(0) * (brush_op / 100.0)
                        
                        # KORA
                        mask_tensor = torch.maximum(mask_tensor, drawn_tensor)
                    except Exception as e:
                        print(f"Kora Brush Mask Gen Error: {e}")
        ...
        # --- KORA SKETCH RENDER ---
        sketch_overlay = Image.new("RGBA", (width, height), (0, 0, 0, 0))
        
        if sketch_data and sketch_data.startswith("data:image"):
            try:
                header, encoded = sketch_data.split(",", 1)
                img_data = base64.b64decode(encoded)
                drawn_img = Image.open(BytesIO(img_data)).convert("RGBA")
                if drawn_img.size != (width, height):
                    drawn_img = drawn_img.resize((width, height), Image.LANCZOS)
                sketch_overlay = drawn_img
            except Exception as e:
                print(f"Kora Sketch Parsing Error: {e}")

        if bg is not None:
            sketch_bg = bg.copy().resize((width, height), Image.LANCZOS).convert("RGBA")
        else:
            sketch_bg = Image.new("RGBA", (width, height), (0, 0, 0, 255))

        sketch_bg = Image.alpha_composite(sketch_bg, sketch_overlay)
        sketch_image_np = np.array(sketch_bg.convert("RGB")).astype(np.float32) / 255.0
        sketch_image_tensor = torch.from_numpy(sketch_image_np).unsqueeze(0)
        return io.NodeOutput(_dumps(caption), preview, bboxes_out, width, height, mask_tensor, sketch_image_tensor, ui=ui)
